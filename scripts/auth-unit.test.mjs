...
test('email authentication redirects accept only known local destinations', () => {
  for (const path of ['/account','/profile','/admin','/admin/events/abc/appearance','/e/adorne-nails-exhibition/claim','/scan/main-entrance']) assert.equal(safeNext(path),path);
