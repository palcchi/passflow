...
export default async function ClaimPage({ params, searchParams }: ClaimPageProps) {
  ...
  const values: Record<string, string> = { name: attendee?.name ?? "", category: ticketName, code: attendee?.attendee_code ?? credential?.display_code ?? "", event_name: event.name, event_date: event.dateLabel, venue: event.venue };
  ...
  return <main className="center-page" style={themeStyle}><div className="center-page-inner">
    ...
    <div className="page-intro"><span className="section-kicker">{event.name}</span><h1>{credential ? "Your event pass." : attendee ? "Connect your wristband." : "Reserve your event pass."}</h1><p>{credential ? "QR pass digitalmu siap dipakai di event." : "Semua akses event akan terhubung ke satu identitasmu."}</p></div>
    ...
    : credential ? <section className="claim-card"><div className="claim-card-top"><span className="section-kicker">{event.name}</span><span className="claim-status claimed">Active</span></div>
      ...
      <form action={replaceQr} className="mt-6"><input type="hidden" name="event_slug" value={slug}/><WristbandInput/><button className="button button-ghost mt-3 w-full" type="submit">Ganti wristband</button></form>
