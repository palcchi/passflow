...
export default async function EventDesignPage({ params, searchParams }: { params: Promise<{ eventId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ data: connection, error: connectionError }, { data: designs, error: designsError }] = await Promise.all([
    supabase.from("figma_connections").select("handle,email,updated_at").eq("user_id", user.id).maybeSingle(),
    supabase.from("event_designs").select("id,kind,name,figma_file_url,figma_file_name,figma_version,preview_url,last_synced_at").eq("event_id", eventId).order("updated_at", { ascending: false }),
  ]);
  ...
  return <main className="editor-page">
    ...
    {!connection ? <section className="design-connect-card"><div className="design-connect-icon"><Figma size={25}/></div><div><span className="section-kicker">Figma account</span><h2>Connect your Figma</h2><p>Login dengan akun Figma organizer. PassFlow hanya membaca metadata, frame, dan preview yang kamu izinkan.</p></div><a className="button button-dark" href={`/api/figma/connect?next=${encodeURIComponent(`/admin/events/${eventId}/design`)}`}>Connect Figma</a></section> : <section className="design-connect-card is-connected"><div className="design-connect-icon"><Figma size={25}/></div><div><span className="section-kicker">Connected</span><h2>{connection.handle ?? connection.email ?? "Figma account"}</h2><p>{connection.email ?? "Akun Figma terhubung ke akun ini."}</p></div><Link className="button button-ghost" href="/profile">Kelola koneksi</Link></section>}
    {(error || connectionError || designsError || (query.figma && query.figma !== "connected")) && <p role="alert" className="design-alert">Figma belum berhasil diproses. Periksa koneksi akun dan akses file, lalu coba lagi.</p>}
    ...
    <section className="design-workspace"><div className="section-heading"><div><span className="section-kicker">Event assets</span><h2>Design library</h2></div><span className="soft-badge">{designs?.length ?? 0} assets</span></div><div className="design-grid">{(designs ?? []).map((design) => <article className="design-card" key={design.id}>{design.preview_url ? <img src={design.preview_url} alt="" /> : <div className="design-preview-empty"><Figma size={26}/><span>Preview belum tersedia</span></div>}<div className="design-card-body"><span className="section-kicker">{assetTypes.find(([value]) => value === design.kind)?.[1] ?? design.kind}</span><h3>{design.name}</h3><p>{design.figma_file_name ?? "Figma file"}{design.figma_version ? ` · ${design.figma_version.slice(0, 8)}` : ""}</p><div className="design-card-actions"><a href={design.figma_file_url} target="_blank" rel="noreferrer" className="button button-ghost"><ExternalLink size={14}/> Edit in Figma</a><form action={syncFigmaDesign}><input type="hidden" name="eventId" value={eventId}/><input type="hidden" name="designId" value={design.id}/><input type="hidden" name="assetType" value={design.kind}/><input type="hidden" name="name" value={design.name}/><input type="hidden" name="figmaUrl" value={design.figma_file_url}/><button className="button button-ghost" type="submit"><RefreshCw size={14}/> Sync</button></form><form action={deleteFigmaDesign}><input type="hidden" name="eventId" value={eventId}/><input type="hidden" name="designId" value={design.id}/><button className="icon-button danger" type="submit" aria-label="Hapus desain"><Trash2 size={15}/></button></form></div></div></article>)}</div>{connection && <form action={syncFigmaDesign} className="design-form"><input type="hidden" name="eventId" value={eventId}/><label>Jenis asset<select name="assetType" defaultValue="id_card">{assetTypes.map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Nama desain<input name="name" required placeholder="VIP ID Card" /></label><label className="design-form-url">Figma file atau frame URL<input name="figmaUrl" required placeholder="https://www.figma.com/design/..." /></label><button className="button button-dark" type="submit"><RefreshCw size={16}/> Sync design</button><p className="text-xs text-muted-foreground">Satu desain per jenis asset. Sync jenis yang sama akan memperbarui desain sebelumnya.</p></form>}</section>
