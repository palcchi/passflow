...
export async function GET(_request: Request, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  const cards = await Promise.all((credentials ?? []).map(async (credential, index) => {
    ...
    const x = (index % columns) * (cardWidth + gap); const y = Math.floor(index / columns) * (cardHeight + gap);
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><title>${escapeXml(event.name)} PassFlow QR export</title>${cards.join("")}</svg>`;
