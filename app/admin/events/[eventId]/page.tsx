        ...
        <section id="access" className="mt-8 rounded-lg border border-border bg-card p-5 sm:p-7">
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-3">
            ...
            <div>
              ...
              <form action={createAccessRule} className="mt-3 space-y-2"><input type="hidden" name="eventId" value={event.id}/><select className={inputClass()} name="zoneId" required><option value="">Zone</option>{zones.map(z=><option key={z.id} value={z.id}>{z.name}</option>)}</select><select className={inputClass()} name="ticketTypeId" required><option value="">Pass type</option>{tickets.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select><select className={inputClass()} name="allowed"><option value="true">Allow</option><option value="false">Deny</option></select><button className="button button-ghost w-full" type="submit">Save rule</button></form>
            </div>
            <div>
              ...
              <form action={createStation} className="mt-3 space-y-2"><input type="hidden" name="eventId" value={event.id}/><input className={inputClass()} name="name" placeholder="Main Entrance" required/><input className={inputClass()} name="slug" placeholder="main-entrance"/><select className={inputClass()} name="mode"><option value="check_in">Check-in</option><option value="zone_access">Zone access</option><option value="activity">Activity</option><option value="claim">Benefit claim</option></select><select className={inputClass()} name="zoneId"><option value="">No zone</option>{zones.map(z=><option key={z.id} value={z.id}>{z.name}</option>)}</select><input className={inputClass()} name="activityCode" placeholder="Activity code, optional"/><input className={inputClass()} name="benefitCode" placeholder="Benefit code, optional"/><button className="button button-dark w-full" type="submit">Create station</button></form>
            </div>
          </div>
        </section>
