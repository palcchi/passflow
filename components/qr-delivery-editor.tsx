...
export function QrDeliveryEditor({ event }: { event: PassFlowEvent }) {
  ...
  return <section className="mt-8 rounded-lg border border-border bg-card p-5 sm:p-7">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <div className="qr-config-preview" style={{ aspectRatio: selected.ratio }}>{templateUrl && <img src={templateUrl} alt="Template pass"/>}<div className="qr-config-preview-mark" style={{ left: `${qrX}%`, top: `${qrY}%`, width: `${qrSize}%` }}>QR</div></div>
    </div>
    ...
    <p className="mt-4 text-xs text-muted-foreground">Template disimpan sebagai gambar dasar. Hasil export tetap berupa HTML siap print dan bisa diimpor ke Figma sebagai layout acuan. Jalur Figma native dapat ditambahkan setelah format brand final.</p>
  </section>;
}
